...
# =============================================================================
# Neuro Ingest & Recall — PAYLOAD VALIDATION (NETAPP)
#   Usage:  bash scripts/neuro_payload_validation_netapp.sh
...
LOGDIR="reports/neuro-payload-validation-netapp/$TS"
mkdir -p "$LOGDIR"
...
log "============================================================"
log "Neuro Payload Validation — NETAPP"
...
LAST_BODY=""; LAST_CODE=""
FAILED_TESTS=()        # collect names of every failed assertion
SKIPPED_TESTS=()       # collect names of every skipped test
INVALIDATION_LOG=()    # track invalidation verification steps
...
# -- XF-1: X-Fabric omission — netapp is single-fabric, so this is a no-op test --
# On single-fabric tenants, omitting X-Fabric still works. Just verify ingest to
# nonexistent space returns 404 (the space doesn't exist, not a fabric issue).
call_no_fabric XF-1_no_fabric POST "$NEURO/v1/spaces/nonexistent-space-xf-test-$(date -u +%s)/ingest" \
  '{"content":"Testing ingest to nonexistent space","threadId":"xf-test"}'
assert_code "XF-1 nonexistent space → 404" 404
