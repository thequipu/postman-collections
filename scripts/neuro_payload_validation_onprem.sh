...
# =============================================================================
# Neuro Ingest & Recall — PAYLOAD VALIDATION (ONPREM)
#   Usage:  bash scripts/neuro_payload_validation_onprem.sh
...
log "============================================================"
log "Neuro Payload Validation — ONPREM"
...
if [ "$LAST_CODE" = "404" ]; then
  ...
  printf '\033[1;33m  ⊘ XF-1 got %s — single-fabric tenant (X-Fabric not required)\033[0m\n' "$LAST_CODE"
  log "  SKIP: XF-1 single-fabric tenant (X-Fabric not required, got $LAST_CODE)"
  ((SKIP++)); SKIPPED_TESTS+=("XF-1: single-fabric tenant, X-Fabric not required")
fi
