...
if [ "$LAST_CODE" = "404" ]; then
  ...
  printf '\033[1;32m  ✓ XF-1 got %s — single-fabric tenant does not require X-Fabric header\033[0m\n' "$LAST_CODE"
  log "  PASS: XF-1 single-fabric tenant (X-Fabric optional, got $LAST_CODE)"
  ((PASS++))
fi
