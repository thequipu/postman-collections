...
def generate():
    ...
    items = [
        ...
        req("09 User Login (reachability)", "POST", "/user/login?isUserAdmin=false",
            ["pm.test('09 Login reachable', () => pm.expect(pm.response.code).to.be.oneOf([200,400,401,500]));",
             "console.log('User login: '+pm.response.code);"],
            base=base, noauth=True,
            extra_headers=[{"key": "X-Tenant-ID", "value": "{{tenant_id}}"}],
            body={"username": "login-reachability-test", "password": "test", "otp": "000000"}),
