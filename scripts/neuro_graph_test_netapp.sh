...
# =============================================================================
# Neuro Graph API — FULL test with DEEP VALIDATIONS (NETAPP)
#   Usage:  bash scripts/neuro_graph_test_netapp.sh
#   Covers: existing APIs + all 16 new graph endpoints
#   Prints every response + assertion results. Writes NOTHING to disk.
# Overridable: SPACE, TENANT, NS, INSECURE
# =============================================================================
...
APP_SVC="https://api-prod-netapp.quipu.netapp.com/applicationService"
SPACE="${SPACE:-neurotest-$(date -u +%Y%m%d%H%M)}"
TENANT="${TENANT:-netapp}"
NS="${NS:-${SPACE}-self}"
INSECURE="${INSECURE:-1}"

USERNAME="vinodrajadmin"; PASSWORD="qGWg9MCPYg7r"
...
log "============================================================"
log "Neuro Graph API Test — NETAPP"
...
# G5. Verify space deleted — ingest on netapp auto-recreates, so handle both
...
if [ "$LAST_CODE" = "404" ]; then
  printf '\033[1;32m  ✓ space ingest blocked after delete — HTTP 404\033[0m\n'
  log "  PASS: space ingest blocked after delete — HTTP 404"
  ((PASS++))
elif [ "$LAST_CODE" = "202" ]; then
  printf '\033[1;33m  ⚠ space ingest auto-recreated space — HTTP 202 (netapp auto-provision)\033[0m\n'
  log "  WARN: space ingest auto-recreated space — HTTP 202 (netapp auto-provision)"
  ((SKIP++))
  echo ">> cleaning up auto-recreated space..." >&2
  call cleanup_auto_recreated DELETE "$APP_SVC/space/by-name/$SPACE"
else
  printf '\033[1;31m  ✗ space ingest unexpected — HTTP %s\033[0m\n' "$LAST_CODE"
  log "  FAIL: space ingest unexpected — HTTP $LAST_CODE"
  ((FAIL++))
fi
