...
# =============================================================================
# Neuro Graph API — FULL test with DEEP VALIDATIONS (PRESTAGE)
#   Usage:  bash scripts/neuro_graph_test_prestage.sh
...
log "============================================================"
log "Neuro Graph API Test — PRESTAGE"
...
if [ "$PROFILE_INERT" = "true" ]; then
  printf '\033[1;36m  ℹ profileInert=true — prompt %s reports no profile slots (profile may still steer via other path)\033[0m\n' "$PROMPT_VERSION"
  log "  INFO: profileInert=true — prompt $PROMPT_VERSION (profile steering verified by entity types)"
else
...
if [ "$FRESH_SPACE" = "true" ]; then
  echo ">> waiting for ingests to project (retry loop, need entities >= 6)..." >&2
  for _ia in $(seq 1 24); do
    sleep 10
    _ic=$($CURL -w $'\n%{http_code}' "$NEURO/v1/spaces/$SPACE/graph/stats?namespaceId=$NS" \
      -H "Authorization: Bearer $TOKEN" -H "X-Tenant-ID: $TENANT" -H "X-Fabric: $FABRIC")
    if [ "${_ic##*$'\n'}" = "401" ]; then refresh_token; continue; fi
    _ie=$(printf '%s' "${_ic%$'\n'*}" | JQ -r '[.labels[] | select(.label=="Entity") | .count] | .[0] // 0' 2>/dev/null)
    echo ">> ingest projection check $_ia/24: entities=$_ie" >&2
    [ "${_ie:-0}" -ge 6 ] 2>/dev/null && break
  done
  refresh_token
else
...
if [ "$FRESH_SPACE" = "true" ]; then
  echo ">> waiting for asserts to project (retry loop, need facts >= 6)..." >&2
  for _aa in $(seq 1 18); do
    sleep 10
    _ac=$($CURL -w $'\n%{http_code}' "$NEURO/v1/spaces/$SPACE/graph/stats?namespaceId=$NS" \
      -H "Authorization: Bearer $TOKEN" -H "X-Tenant-ID: $TENANT" -H "X-Fabric: $FABRIC")
    if [ "${_ac##*$'\n'}" = "401" ]; then refresh_token; continue; fi
    _ae=$(printf '%s' "${_ac%$'\n'*}" | JQ -r '[.labels[] | select(.label=="Fact") | .count] | .[0] // 0' 2>/dev/null)
    echo ">> assert projection check $_aa/18: facts=$_ae" >&2
    [ "${_ae:-0}" -ge 6 ] 2>/dev/null && break
  done
  refresh_token
else
...
log "  FACTS LISTED:"
printf '%s' "$LAST_BODY" | JQ -r '.items[] | "    [\(.name)] \(.sourceNodeName) → \(.targetNodeName): \(.fact)"' 2>/dev/null >> "$LOGFILE"
# Use the fully-populated fact for EDGE_URI (not items[0] which might have null sourceNodeName)
...
# ---- Invalidation test: LIVE mode should NOT return invalidated facts ----
# Use EDGE_URI_2 (second fact) — invalidate it, then check recall
if [ -n "${EDGE_URI_2:-}" ]; then
  # Get the fact text before invalidation (for logging)
  call get_fact_before_invalidate GET "$NEURO/v1/spaces/$SPACE/graph/edge?uri=$(uriencode "$EDGE_URI_2")"
  INVALIDATE_FACT_TEXT=$(printf '%s' "$LAST_BODY" | JQ -r '.fact // "unknown"' 2>/dev/null)
  echo ">> invalidating fact: $INVALIDATE_FACT_TEXT" >&2
  log "  INFO: invalidating EDGE_URI_2=$EDGE_URI_2 fact=$INVALIDATE_FACT_TEXT"

  # Patch with invalidAt — marks fact as "no longer true"
  call invalidate_fact PATCH "$NEURO/v1/spaces/$SPACE/graph/edge?uri=$(uriencode "$EDGE_URI_2")" \
    '{"invalidAt":"2026-01-01T00:00:00Z"}'
  assert_code "invalidate fact (set invalidAt)" 202

  # Wait for invalidation to propagate to recall index
  echo ">> sleeping 30s for invalidation to propagate to recall index..." >&2
  sleep 30
  refresh_token

  # Verify the fact now has invalidAt set
  wait_for verify_invalidated 200 "$NEURO/v1/spaces/$SPACE/graph/edge?uri=$(uriencode "$EDGE_URI_2")" 6
  assert_code "invalidated fact readable" 200
  INVALID_AT=$(printf '%s' "$LAST_BODY" | JQ -r '.invalidAt // empty' 2>/dev/null)
  if [ -n "$INVALID_AT" ] && [ "$INVALID_AT" != "null" ]; then
    printf '\033[1;32m  ✓ fact has invalidAt set — %s\033[0m\n' "$INVALID_AT"
    log "  PASS: fact has invalidAt=$INVALID_AT"
    ((PASS++))
  else
    printf '\033[1;33m  ⚠ invalidAt not yet visible on fact\033[0m\n'
    log "  WARN: invalidAt not yet visible"
    ((SKIP++))
  fi

  # Recall LIVE with includeInvalidated=false — invalidated fact should NOT appear
  call recall_live_after_invalidate POST "$NEURO/v1/memories/$NS/recall" \
    '{"query":"'"$(printf '%s' "$INVALIDATE_FACT_TEXT" | head -c 80)"'","tokenBudget":1200,"mode":"LIVE","includeInvalidated":false}'
  assert_code "recall LIVE after invalidation" 200
  # Check invalidated fact in recall with includeInvalidated=false — should be excluded
  INVALID_ITEMS=$(printf '%s' "$LAST_BODY" | JQ -r '[.items[] | select(.invalidAt != null)] | length' 2>/dev/null)
  INVALID_IN_PROV=$(printf '%s' "$LAST_BODY" | JQ -r --arg uri "$EDGE_URI_2" '[.items[] | select(.provenance[]? == $uri)] | length' 2>/dev/null)
  if [ "${INVALID_IN_PROV:-0}" = "0" ] && [ "${INVALID_ITEMS:-0}" = "0" ]; then
    printf '\033[1;32m  ✓ LIVE recall with includeInvalidated=false excludes invalidated facts\033[0m\n'
    log "  PASS: LIVE recall with includeInvalidated=false excludes invalidated facts"
    ((PASS++))
  elif [ "${INVALID_IN_PROV:-0}" = "0" ] && [ "${INVALID_ITEMS:-0}" -gt 0 ] 2>/dev/null; then
    printf '\033[1;33m  ⚠ LIVE recall has %s items with invalidAt set (other facts, not our target)\033[0m\n' "$INVALID_ITEMS"
    log "  WARN: LIVE recall has $INVALID_ITEMS items with invalidAt (other facts)"
    ((SKIP++))
  else
    printf '\033[1;31m  ✗ BUG: LIVE recall with includeInvalidated=false still returns invalidated fact (invalidAt items=%s)\033[0m\n' "$INVALID_ITEMS"
    log "  FAIL: BUG: LIVE recall with includeInvalidated=false still returns invalidated fact"
    ((FAIL++))
  fi

  # Recall LIVE with includeInvalidated=true — fact SHOULD appear
  call recall_live_include_invalidated POST "$NEURO/v1/memories/$NS/recall" \
    '{"query":"'"$(printf '%s' "$INVALIDATE_FACT_TEXT" | head -c 80)"'","tokenBudget":1200,"mode":"LIVE","includeInvalidated":true}'
  assert_code "recall LIVE includeInvalidated=true" 200
  INCL_FOUND=$(printf '%s' "$LAST_BODY" | JQ -r --arg uri "$EDGE_URI_2" '[.items[] | select(.provenance[]? == $uri)] | length' 2>/dev/null)
  if [ "${INCL_FOUND:-0}" -gt 0 ] 2>/dev/null; then
    printf '\033[1;32m  ✓ includeInvalidated=true returns the invalidated fact\033[0m\n'
    log "  PASS: includeInvalidated=true returns the invalidated fact"
    ((PASS++))
  else
    printf '\033[1;33m  ⚠ includeInvalidated=true did not return invalidated fact (projection lag)\033[0m\n'
    log "  WARN: includeInvalidated=true did not return invalidated fact"
    ((SKIP++))
  fi

  # Recall AS_OF (before invalidAt) — fact SHOULD appear (it was valid at that time)
  call recall_asof_before_invalidate POST "$NEURO/v1/memories/$NS/recall" \
    '{"query":"'"$(printf '%s' "$INVALIDATE_FACT_TEXT" | head -c 80)"'","tokenBudget":1200,"mode":"AS_OF","asOf":"2025-06-01T00:00:00Z"}'
  assert_code "recall AS_OF before invalidAt" 200
  log "  INFO: AS_OF recall items count: $(printf '%s' "$LAST_BODY" | JQ -r '.items | length' 2>/dev/null)"
else
  skip "invalidation test" "no EDGE_URI_2 from listings"
fi
...
if [ -n "$EDGE_URI" ]; then
  ...
  wait_for verify_fact_deleted 404 "$NEURO/v1/spaces/$SPACE/graph/edge?uri=$(uriencode "$EDGE_URI")" 36
  refresh_token
...
if [ -n "$CREATED_NODE_URI" ] && [ "$NODE_PROJECTED" = "true" ]; then
  ...
  wait_for verify_node_deleted 404 "$NEURO/v1/spaces/$SPACE/graph/node?uri=$(uriencode "$CREATED_NODE_URI")&namespaceId=$NS" 36
...
if [ -n "$EPISODE_URI" ]; then
  ...
  wait_for verify_episode_deleted 404 "$NEURO/v1/spaces/$SPACE/graph/episode?uri=$(uriencode "$EPISODE_URI")" 36
